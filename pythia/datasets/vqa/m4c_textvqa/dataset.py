...
from pythia.utils.myUtils import create_obj_labels, findRelatedOCR_adr, findRelatedOCR_ori,  object_bert_ids, add_ocr_char_info, create_ocr_char_info,findRelatedOCR, add_cons_ocr_info,create_same_labels
...
class M4CTextVQADataset(TextVQADataset):
    ...
    def format_for_evalai(self, report):
        ...
        for idx, question_id in enumerate(report.question_id):
            ...
            try:
                ...
                print(idx)
                print(len(report.context_tokens))
            answer_words = []
            pred_source = []
    ...
    def add_sample_details(self, sample_info, sample):
        ...
        #processed_question = self.char_text_processor({"question": question_str}, updatelen=20)
        if self.pretrain:
        ...
        #sample.obj_labels = create_obj_labels(sample['image_info_0']['object_tokens'],100)
        ...
        if self.msocr:   ## load ms ocr prediction directly from _info.npy; else: from imdb provided
            ...
            if len(sample.image_info_1["ocr_tokens"])>max_len:
                sample.image_info_1['ocr_tokens'], sample.image_info_1['ocr_boxes'], sample.image_info_1['ocr_conf'] = \
            ...
            sample.ocr_conf = self.copy_processor(
                {"blob": np.array(sample.image_info_1['ocr_conf'], dtype=np.float32)}
            )["blob"][:max_len]
            if 'recover' in self.config.ocr.features:
                if 'recover_words' not in sample_info:
                    recover_ocr_tokens = None
                else:
                    recover_ocr_tokens = [
                        self.ocr_token_processor({"text": token})["text"]
                        for token in sample_info["recover_words"]]
        else:
        ...
        #######################################################################
        ## generate augmented OCR Tokens and TWC labels
        ## Load OCR bert token
        ## Get char info for OCR tokens
        if ('char' in self.config.ocr.features or 'bert' in self.config.ocr.features) and 'cons' in self.config.ocr.features:
            adv_pro = self.config.ocr.adv_pro
            if 'editlen' in self.config.ocr:
                editlen = self.config.ocr.editlen
            else:
                editlen = 2
            if 'label' in self.config.ocr.features and self.pretrain:
                label_list = self.config.ocr.label_list
                pad_ocr_tokens, related_ocr_tokens, sample.o2r_labels, sample.r2o_labels = findRelatedOCR_adr(
                    ocr_tokens, ocr_str_len, adv_pro, label_list, editlen)
                #pad_ocr_tokens, related_ocr_tokens, sample.o2r_labels, sample.r2o_labels = findRelatedOCR_ori(
                #    ocr_tokens, ocr_str_len, adv_pro, label_list, editlen)
            else:
                pad_ocr_tokens, related_ocr_tokens = findRelatedOCR(ocr_tokens, ocr_str_len, adv_pro, editlen)
            if 'recover' in self.config.ocr.features:
                if recover_ocr_tokens != None:
                    print("use recover words")
            ...
            sample.word_ids = torch.cat([word_ids,related_word_ids],dim=0)
            pad_ocr_tokens = pad_ocr_tokens + related_ocr_tokens
        elif 'char' in self.config.ocr.features or 'bert' in self.config.ocr.features:
            context_char, context_char_mask, word_ids = add_cons_ocr_info(ocr_tokens, ocr_str_len)
        ...
        ## but truncated to ocr_str_len anyway
        ...
        #print(pad_ocr_tokens)
        context = self.context_processor({"tokens": pad_ocr_tokens})
        sample.context = context["text"]
        sample.context_tokens = context["tokens"]
        sample.context_tokens_enc = enc_obj2bytes(context["tokens"])
        sample.context_feature_0 = context["text"]
        sample.context_info_0 = Sample()
        token_length = torch.tensor(min(len(ocr_tokens), ocr_str_len), dtype=torch.long)
        sample.context_info_0.max_features = token_length #context_ori["length"]
        #print(sample.context_info_0.max_features)
        # Get PHOC embeddings for OCR tokens
        #context_phoc_ori = self.phoc_processor({"tokens": ocr_tokens})
        context_phoc = self.phoc_processor({"tokens": pad_ocr_tokens})
        sample.context_feature_1 = context_phoc["text"]
        sample.context_info_1 = Sample()
        sample.context_info_1.max_features = token_length  #context_phoc_ori["length"]
        # OCR order vectors
    ...
    def ocr_truncate(self, token_array, ocr_boxes, conf_array=None, mode='naive'):
        ...
        return token_array[idx].tolist(), ocr_boxes[idx,:], conf_array[idx]
