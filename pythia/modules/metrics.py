...
class PreTrainMLMAccuracy(BaseMetric):
    def __init__(self):
        ...
        self.charpred_accuracy = PreTrainCharAccuracy()
        self.conspred_accuracy = PreTrainConsAccuracy()
        self.recover_accuracy = PreTrainRecoverAccuracy()
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        if "term_scores" in model_output:
            return (accuracy + self.charpred_accuracy.calculate(sample_list, model_output)
            + self.contrapred_accuracy.calculate(sample_list, model_output)
            + self.rpp_accuracy.calculate(sample_list, model_output))/4.
        elif "recover_scores" in model_output:
            recover_acc = self.recover_accuracy.calculate(sample_list, model_output)
            print(recover_acc)
            return (accuracy + recover_acc            
            + self.contrapred_accuracy.calculate(sample_list, model_output)
            + self.rpp_accuracy.calculate(sample_list, model_output))/4.
        else:
            return (accuracy
            + self.contrapred_accuracy.calculate(sample_list, model_output)
            + self.rpp_accuracy.calculate(sample_list, model_output))/3.

@registry.register_metric("recover_accuracy")
class PreTrainRecoverAccuracy(BaseMetric):
    def __init__(self):
        super().__init__("recover_accuracy")

    def calculate(self, sample_list, model_output, *args, **kwargs):
        batch_size, tokenlen, vocab_size = model_output["recover_scores"].shape
        scores = model_output["recover_scores"].view(batch_size * tokenlen, -1)
        targets = sample_list["recover_labels"].view(batch_size * tokenlen)
        loss_mask = (targets != -1).float().bool()
        if loss_mask.float().sum()==0.:
            accuracy = torch.zeros(1).cuda()
        else:
            scores, targets = scores[loss_mask], targets[loss_mask]
            accuracy = torch.sum(scores.argmax(1)==targets).float()/max(targets.shape[0],1)
        return accuracy
@registry.register_metric("conspred_accuracy")
class PreTrainConsAccuracy(BaseMetric):
    def __init__(self):
        super().__init__("conspred_accuracy")

    def calculate(self, sample_list, model_output, *args, **kwargs):
        b1, b2 = model_output["contrastive_scores"].shape
        scores = model_output["contrastive_scores"] #.view(b1, -1)
        targets = torch.arange(0, b1).to(device=model_output["contrastive_scores"].device)
        #sample_list["adv_input_labels"] #.view(batch_size * tokenlen)
        loss_mask = (targets != -1).float().bool()
        if loss_mask.float().sum()==0.:
            accuracy = torch.zeros(1).cuda()
        else:
...
@registry.register_metric("charpred_accuracy")
class PreTrainCharAccuracy(BaseMetric):
    def __init__(self):
        super().__init__("charpred_accuracy")

    def calculate(self, sample_list, model_output, *args, **kwargs):
        batch_size, tokenlen, vocab_size = model_output["term_scores"].shape
        scores = model_output["term_scores"].view(batch_size * tokenlen, -1)
        targets = sample_list["adv_input_labels"].view(batch_size * tokenlen)
        loss_mask = (targets != -1).float().bool()
        if loss_mask.float().sum()==0.:
            accuracy = torch.zeros(1).cuda()
        else:
