...
class Checkpoint:
    ...
    def _load(self, file, force=False):
        ...
        ## temporal solution: training from ep 0 in refine
        ## do not reset if resume pretraining (mainly for continue experiments on azure failure)
        if ('pretrain' in file) and '_refine_' in self.save_dir:
            ckpt['best_metric_value'] = 0.
            print('--------------\nWARNING: best_metric_value hard resets to 0\nfor model %s\n--------------'%file)
        ...
        if len(pretrained_mapping.items()) == 0:
            ...
            if "best_iteration" in ckpt:
                ...
                if ('pretrain' in file) and '_refine_' in self.save_dir:
                    ...
                    print('--------------\nWARNING: current_iteration hard resets to 0\nfor model %s\n--------------'%file)
